const C='mystery-book-studio-v24';
self.addEventListener('install',e=>{self.skipWaiting();e.waitUntil(caches.open(C).then(c=>c.addAll(['./','./index.html','./app-v17.js','./swatch-v19.js','./orientation-v21.js','./manual-rotate-v22.js','./pdf-import-v23.js','./manifest.webmanifest'])))});
self.addEventListener('activate',e=>e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k!==C).map(k=>caches.delete(k)))).then(()=>self.clients.claim())));
async function appBundle(req){
 const base=await fetch(req,{cache:'no-store'}),swatch=await fetch(new URL('./swatch-v19.js',self.location),{cache:'no-store'}),orient=await fetch(new URL('./orientation-v21.js',self.location),{cache:'no-store'}),rotate=await fetch(new URL('./manual-rotate-v22.js',self.location),{cache:'no-store'}),pdf=await fetch(new URL('./pdf-import-v23.js',self.location),{cache:'no-store'});
 if(!base.ok||!swatch.ok||!orient.ok||!rotate.ok||!pdf.ok)throw new Error('bundle fetch failed');
